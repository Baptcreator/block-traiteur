    ...
    private function display_wine_subcategories() {
        ...
        <div class="wrap">
            ...
            if (isset($_GET['message'])) {
                switch ($_GET['message']) {
                    case 'type_created':
                        $new_type = isset($_GET['new_type']) ? sanitize_text_field($_GET['new_type']) : 'inconnu';
                        echo '<div class="notice notice-success"><p>✅ Type de vin "' . esc_html(ucfirst($new_type)) . '" créé avec succès !</p></div>';
                        break;
                }
            }
            ...
            if (isset($_GET['error'])) {
                switch ($_GET['error']) {
                    case 'save_failed':
                        $debug = isset($_GET['debug']) ? sanitize_text_field($_GET['debug']) : '';
                        echo '<div class="notice notice-error"><p>❌ Erreur lors de la sauvegarde. ' . ($debug ? 'Détails: ' . esc_html($debug) : 'Vérifiez la base de données.') . '</p></div>';
                        break;
                    case 'already_exists':
                        echo '<div class="notice notice-warning"><p>⚠️ Ce type de vin existe déjà dans la base de données.</p></div>';
                        break;
                    case 'invalid_slug':
                        echo '<div class="notice notice-error"><p>❌ Le nom du type contient des caractères invalides. Utilisez uniquement des lettres et des espaces.</p></div>';
                        break;
                    case 'empty_name':
                        echo '<div class="notice notice-error"><p>❌ Le nom du type ne peut pas être vide.</p></div>';
                        break;
                    case 'type_in_use':
                        $usage_count = isset($_GET['usage_count']) ? intval($_GET['usage_count']) : 0;
                        echo '<div class="notice notice-error"><p>❌ Ce type ne peut pas être supprimé car il est utilisé par ' . $usage_count . ' produit(s).</p></div>';
                        break;
                }
            }
            ?>
