...
class RestaurantBooking_Ajax_Handler_V3
{
    ...
    /**
     * ✅ CORRECTION : Headers anti-cache pour réponses AJAX
     */
    private function send_ajax_headers()
    {
        // Entêtes anti-cache strictes
        header('Cache-Control: no-cache, no-store, must-revalidate, max-age=0');
        header('Cache-Control: post-check=0, pre-check=0', false);
        header('Pragma: no-cache');
        header('Expires: Thu, 01 Jan 1970 00:00:00 GMT');
        
        // Entêtes spécifiques pour les CDN/WAF
        header('Vary: User-Agent, Accept-Encoding');
        
        // Supprimer les entêtes de cache du serveur
    ...
    /**
     * ✅ CORRECTION : Nettoyage complet de la sortie avant JSON
     */
    private function clean_output_for_json()
    {
        // Supprimer tous les output buffers existants
        while (ob_get_level()) {
            ob_end_clean();
        }
        
        // Nettoyer toute sortie émise avant (erreurs PHP, whitespace, etc.)
        if (function_exists('error_clear_last')) {
            error_clear_last();
        }
        
        // S'assurer qu'aucun BOM ou caractère invisible n'est présent
        $this->remove_bom_whitespace();
    }

    /**
     * ✅ CORRECTION : Suppression du BOM et whitespace
     */
    private function remove_bom_whitespace()
    {
        // Vérifier si on est en début de fichier (pas de sortie autorisée)
        if (!headers_sent()) {
            return true; // Aucune sortie avant les headers = OK
        }
        
        // Si des headers sont déjà envoyés, on nettoie ce qui est dans le buffer
        $content = '';
        while (ob_get_level()) {
            $content .= ob_get_clean();
        }
        
        // Supprimer BOM UTF-8 et whitespace en début/fin
        $content = preg_replace('/^\xEF\xBB\xBF/', '', $content); // BOM UTF-8
        $content = trim($content);
        
        // Si il reste du contenu non-JSON, le logguer pour debug
        if (!empty($content) && !preg_match('/^\s*\{/', $content)) {
            error_log('Restaurant Booking V3: Sortie PHP avant JSON détectée: ' . substr($content, 0, 100));
        }
        
        return empty($content) || preg_match('/^\s*\{/', $content);
    }

    /**
     * ✅ CORRECTION : Debug info pour diagnostiquer problèmes utilisateurs non connectés
     */
    private function log_ajax_debug_info($action)
    {
        $debug_info = [
        ...
        error_log('Restaurant Booking V3 DEBUG: ' . json_encode($debug_info));
    }

    /**
     * ✅ CORRECTION : Wrapper pour réponses JSON avec headers anti-cache
     */
    private function send_json_response($success, $data, $status_code = null)
    {
        // ✅ CORRECTION : Nettoyage complet avant réponse JSON
        $this->clean_output_for_json();
        
        // Envoyer les headers anti-cache
        $this->send_ajax_headers();
        
        // Assurer un Content-Type propre
        header('Content-Type: application/json; charset=utf-8');
        
        // Envoyer la réponse JSON avec wp_send_json_*
        if ($success) {
    ...
    public function load_step()
    {
        ...
        try {
            ...
            error_log('Restaurant Booking V3 load_step: ' . $e->getMessage());
            $this->send_json_response(false, ['message' => 'Erreur lors du chargement de l\'étape']);
        }
    }
    ...
    public function submit_quote()
    {
        ...
        try {
            ...
            if ($quote_id) {
                ...
                $this->send_json_response(true, ['quote_id' => $quote_id]);
            } else {
                $this->send_json_response(false, ['message' => 'Erreur lors de la création du devis']);
            }
        } catch (Exception $e) {
            error_log('Restaurant Booking V3 submit_quote: ' . $e->getMessage());
            $this->send_json_response(false, ['message' => 'Erreur lors de l\'envoi du devis']);
        }
    }
    ...
    public function load_signature_products()
    {
        ...
        try {
            ...
            error_log('Restaurant Booking V3 load_signature_products: ' . $e->getMessage());
            $this->send_json_response(false, ['message' => 'Erreur lors du chargement des produits']);
        }
    }
    ...
    public function get_availability()
    {
        try {
            ...
            $this->send_json_response(true, $availability_data);

        } catch (Exception $e) {
            error_log('Restaurant Booking V3 get_availability: ' . $e->getMessage());
    ...
    public function calculate_distance()
    {
        try {
            ...
            if (!wp_verify_nonce($_POST['nonce'] ?? '', 'restaurant_booking_form_v3')) {
                $this->send_json_response(false, ['message' => 'Erreur de sécurité (nonce invalide)']);
                return; // ✅ CORRECTION : Sortir après la réponse d'erreur
            }
            ...
            $this->send_json_response(true, $response_data);

        } catch (Exception $e) {
           error_log('Restaurant Booking V3 calculate_distance: ' . $e->getMessage());
            $this->send_json_response(false, ['message' => $e->getMessage()]);
        }
    }
    ...
    /**
     * ✅ NOUVEAU : Générer le HTML des suppléments pour les buffets
     */
    private function get_buffet_supplements_html($product_id, $buffet_type, $form_data = [], $product_quantity = 0)
    {
        global $wpdb;

        // Récupérer les suppléments depuis wp_restaurant_accompaniment_options
        $supplements = $wpdb->get_results($wpdb->prepare("
            SELECT id, option_name, option_price, display_order
        ...
        $html = '<div class="rbf-v3-buffet-supplements" data-buffet-type="' . esc_attr($buffet_type) . '" data-product-id="' . esc_attr($product_id) . '">';
        $html .= '<h5 class="rbf-v3-supplements-title">Suppléments disponibles :</h5>';
        
        foreach ($supplements as $supplement) {
            $saved_qty = intval($form_data['buffet_' . $buffet_type . '_' . $product_id . '_supplement_' . $supplement->id . '_qty'] ?? 0);
            
            // ✅ CORRECTION : La quantité max des suppléments ne peut pas dépasser la quantité du produit
            $max_quantity = max($product_quantity, 1); // Au moins 1 pour permettre la sélection
            
            $html .= '<div class="rbf-v3-supplement-item">';
            $html .= '<div class="rbf-v3-supplement-info">';
