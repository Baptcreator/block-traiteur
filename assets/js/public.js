...
(function($) {
    ...
    let RestaurantBookingPublic = {
        ...
        selectedOptions: [],
        formData: {},
        getUnifiedAjaxUrl: function() {
            // Priorité : restaurant_booking_ajax > rbfV3Config > rbfV3Ajax
            if (typeof restaurant_booking_ajax !== 'undefined' && restaurant_booking_ajax?.ajax_url) {
                return restaurant_booking_ajax.ajax_url;
            }
            if (typeof rbfV3Config !== 'undefined' && rbfV3Config?.ajaxUrl) {
                return rbfV3Config.ajaxUrl;
            }
            if (typeof rbfV3Ajax !== 'undefined' && rbfV3Ajax?.ajaxUrl) {
                return rbfV3Ajax.ajaxUrl;
            }
            return '/wp-admin/admin-ajax.php';
        },
        
        getUnifiedNonce: function() {
            // Priorité : restaurant_booking_ajax > rbfV3Config > rbfV3Ajax
            if (typeof restaurant_booking_ajax !== 'undefined' && restaurant_booking_ajax?.nonce) {
                return restaurant_booking_ajax.nonce;
            }
            if (typeof rbfV3Config !== 'undefined' && rbfV3Config?.nonce) {
                return rbfV3Config.nonce;
            }
            if (typeof rbfV3Ajax !== 'undefined' && rbfV3Ajax?.nonce) {
                return rbfV3Ajax.nonce;
            }
            return '';
        },
        
        getUnifiedMessages: function() {
            // Priorité : restaurant_booking_ajax > rbfV3Config > rbfV3Ajax
            if (typeof restaurant_booking_ajax !== 'undefined' && restaurant_booking_ajax?.messages) {
                return restaurant_booking_ajax.messages;
            }
            if (typeof rbfV3Config !== 'undefined' && rbfV3Config?.texts) {
                return rbfV3Config.texts;
            }
            return {};
        },

        // ✅ CORRECTION : Configuration unifiée
        getConfig: function() {
            return {
                ajaxUrl: this.getUnifiedAjaxUrl(),
                nonce: this.getUnifiedNonce(),
                messages: this.getUnifiedMessages()
            };
        },

        // ✅ CORRECTION : Parse défensif des réponses AJAX
        ...
        calculateDeliveryDistance: function(postalCode) {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    const parsedResponse = this.parseAjaxResponse(response);
                    ...
                    if (parsedResponse.success && parsedResponse.data) {
                        ...
                        const errorMessage = parsedResponse.data ? (parsedResponse.data.message || parsedResponse.data) : 'Erreur de calcul de distance';
        ...
        submitForm: function(action, formSelector) {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (parsedResponse.success && parsedResponse.data) {
                        const message = parsedResponse.data.message || 'Devis envoyé avec succès !';
                        this.showMessage('✅ ' + message, 'success');
                        this.clearFormData(); // Effacer les données de session
                        
                        // Optionnel : redirection ou affichage du numéro de devis
                        if (parsedResponse.data.quote_id) {
                            this.showMessage(`📋 Numéro de devis : ${parsedResponse.data.quote_id}`, 'info');
                        }
                    } else {
                        const errorMessage = parsedResponse.data ? 
                            (typeof parsedResponse.data === 'string' ? parsedResponse.data : parsedResponse.data.message) : 
